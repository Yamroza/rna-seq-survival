...
python clinical_data/create_clinical_data.py --cohort $COHORT

echo "Splitting data into ${N_FOLDS} folds..."
python clinical_data/split_data_into_folds.py --cohort $COHORT --n_folds $N_FOLDS

print_step "FOUNDATION MODELS: tGPT EMBEDDINGS"
echo "Running tGPT with max_len=${TGPT_MAX_LEN}"
python tgpt/generate_embeddings_tgpt.py --filename $FILENAME --max_len $TGPT_MAX_LEN
...
python scgpt/generate_embeddings_scgpt.py \
    --filename $FILENAME \
